...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        officers = Officer.objects.all().annotate(count=Count('complaint'))
        for officer in officers:
            officer.allegations_count = officer.count
            officer.discipline_count = officer.complaint_set.exclude(final_outcome='600').count()
            officer.save()

        for allegation_cat in AllegationCategory.objects.all():
            allegation_cat.allegations_count = allegation_cat.complaint_set.count()
