            ...
            if (isArray(child)) {
                forEach(child, function (c) {
                    if (c && typeof c.type === 'string') {
                        insertHelpers(c, node, result.chunks);
                        walk(c, node);
                    }
                });
            }
...
window.falafel = module.exports;})(function(moduleName){switch(moduleName){case "esprima": return esprima;
case "acorn":
    ...
    _blanket = {
        ...
        _addTracking: function (filename) {
            ...
            return function(node){
                ...
                if (linesToAddTracking.indexOf(node.type) > -1 && node.parent && node.parent.type !== "LabeledStatement") {
