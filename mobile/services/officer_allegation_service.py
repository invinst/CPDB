...
class OfficerAllegationService(object):
    ...
    def get_officer_allegations(officer_id):
        ...
        officer_allegations = sorted(
            list(officer_allegations.prefetch_related('cat')),
            key=lambda x: x.allegation_id)
        officer_allegations = groupby(
            officer_allegations, lambda x: x.allegation_id)
