...
class EmbedModeTestCase(BaseLiveTestCase):
    ...
    def test_embed_codes(self):
        ...
        map_column_code.get_attribute('value').should.contain('/embed/?page=map&query=&state=%7B%7D')

        tabs_column_code = self.find('.tabs-column .embed-code input')
        tabs_column_code.should.be.ok
        tabs_column_code.get_attribute('value').should.contain('/embed/?page=sunburst&query=&state=%7B%22name%22%3A%22Allegations%22%7D')
    ...
    def test_can_not_change_filter_in_embed_mode(self):
        ...
        self.until(lambda: self.find('.ui-autocomplete-input').send_keys(self.allegation.officer.officer_first))
